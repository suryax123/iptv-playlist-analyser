/**
 * PlaylistPDF - IPTV Playlist Analyzer & PDF Converter
 * Client-side JavaScript with PDF generation
 * 
 * Security: All user inputs validated before use
 * Performance: Lazy loading for jsPDF library
 */
...
// ============================================
// Input Validation Constants (Security)
// ============================================
...
function setupEventListeners() {
  ...
  filterBtns.forEach(btn => {
    btn.addEventListener('click', () => filterChannels(btn.dataset.filter));
  });
...
/**
 * Sanitize URL for display (Security)
 */
...
/**
 * Escape HTML to prevent XSS (Security)
 */
function escapeHtml(text) {
  if (!text) return '';
  const div = document.createElement('div');
  div.textContent = String(text).substring(0, 1000); // Limit length
...
// ============================================
// jsPDF Lazy Loading (Performance)
// ============================================
...
/**
 * Generate Playlist PDF on client-side
 */
async function generatePlaylistPDF(content, sourceUrl) {
  ...
  for (const line of lines) {
    ...
    // Color coding
    if (trimmedLine.startsWith('#EXTM3U') || trimmedLine.startsWith('#EXT')) {
...
/**
 * Generate Analysis Report PDF on client-side
 */
async function generateAnalysisReportPDF(analysis) {
  ...
  const summaryItems = [
    ...
    ['Checked Channels:', summary.checkedChannels],
    ['Live Channels:', `${summary.liveChannels} (${summary.livePercentage}%)`],
    ['Dead Channels:', summary.deadChannels],
  ...
  if (analysis.groups && analysis.groups.length > 0) {
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('Categories/Groups', margin, y);
    y += 8;
    
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    
    analysis.groups.slice(0, 15).forEach(group => {
      if (y > pageHeight - 20) {
        doc.addPage();
        y = margin;
      }
      doc.setTextColor(51, 51, 51);
      doc.text(`• ${escapeHtml(group.name)}: ${group.count} channels`, margin, y);
      y += 5;
    });
  ...
  if (analysis.channels && analysis.channels.length > 0) {
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('Channel Details', margin, y);
    y += 10;
    ...
    analysis.channels.slice(0, 100).forEach((channel, index) => {
      ...
      // Status indicator
      const statusColor = channel.status === 'live' ? [16, 185, 129] : 
                         channel.status === 'dead' ? [239, 68, 68] : [136, 136, 136];
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(51, 51, 51);
      doc.text(`${index + 1}. ${escapeHtml(channel.name || 'Unknown').substring(0, 50)}`, margin + 6, y);
      y += 4;
      ...
      doc.setTextColor(102, 102, 102);
      doc.setFontSize(7);
      let details = `Group: ${escapeHtml(channel.group || 'N/A')} | Status: ${channel.status.toUpperCase()}`;
      if (channel.responseTime) {
        details += ` | ${channel.responseTime}ms`;
      }
...
function updateModeUI() {
  const placeholder = urlInput;
  const actionBtnIcon = document.getElementById('actionBtnIcon');
  
  if (currentMode === 'analyze') {
    actionBtnText.textContent = 'Analyze Playlist';
    loadingText.textContent = 'Analyzing...';
    placeholder.placeholder = 'Paste your playlist URL here (HTTP or HTTPS)...';
    if (actionBtnIcon) {
    ...
    actionBtnText.textContent = 'Check Channel';
    loadingText.textContent = 'Checking...';
    placeholder.placeholder = 'Paste channel/stream URL (e.g., .m3u8, .ts)...';
    if (actionBtnIcon) {
      actionBtnIcon.innerHTML = `
    ...
    actionBtnText.textContent = 'Convert to PDF';
...
function handlePaste(e) {
  ...
  setTimeout(() => {
    if (urlInput.value.length > MAX_URL_LENGTH) {
      urlInput.value = urlInput.value.substring(0, MAX_URL_LENGTH);
      showStatus('Warning', 'URL was truncated to maximum allowed length', 'error');
    }
  }, 0);
}
...
async function handleAction() {
  ...
  if (!url) {
    showStatus('Missing URL', 'Please enter a URL', 'error');
    return;
  }
  
  if (!isValidUrl(url)) {
    showStatus('Invalid URL', 'Please enter a valid URL starting with http:// or https://', 'error');
    return;
  }
  ...
  if (currentMode === 'analyze') {
    ...
    await checkChannel(url);
  } else {
    await convertPlaylist(url);
  }
}
...
async function checkChannel(url) {
  ...
  try {
    ...
    hideStatus();
    displayChannelResults(data, url);
    
  } catch (error) {
    console.error('Channel check error:', error);
    showStatus('Error', error.message || 'Channel check failed', 'error');
  } finally {
    setLoading(false);
  }
}
...
// ============================================
// Display Single Channel Results
// ============================================
...
function displayChannelResults(result, url) {
  ...
  const detailHttpCode = document.getElementById('detailHttpCode');
  ...
  const detailUrlValid = document.getElementById('detailUrlValid');
  ...
  // Details
  detailStatus.textContent = result.status.toUpperCase();
  detailStatus.className = `detail-value ${isLive ? 'live' : 'dead'}`;
  
  detailHttpCode.textContent = result.httpStatus || '-';
  
  const responseTime = result.responseTime || 0;
  detailResponseTime.textContent = `${responseTime}ms`;
  detailResponseTime.className = `detail-value ${getResponseClass(responseTime)}`;
  
  detailContentType.textContent = result.contentType 
    ? (result.contentType.split(';')[0] || '-')
    : '-';
  
  // Extra analysis fields
  if (detailUrlValid) {
    detailUrlValid.textContent = result.urlValid ? '✓ Valid' : '✗ Invalid';
    detailUrlValid.className = `detail-value ${result.urlValid ? 'live' : 'dead'}`;
  }
  
  if (detailProtocol) {
    detailProtocol.textContent = result.protocol || '-';
    detailProtocol.className = `detail-value ${result.protocol === 'HTTPS' ? 'live' : ''}`;
  }
  
  if (detailStreamType) {
    detailStreamType.textContent = result.streamType || '-';
  }
  
  if (detailServer) {
    const server = result.server || '-';
    detailServer.textContent = server.length > 25 ? server.substring(0, 25) + '...' : server;
  }
...
async function convertPlaylist(url) {
  ...
  try {
    ...
    currentPlaylistContent = data.content;
    currentPlaylistUrl = url;
    ...
    showStatus('Error', error.message || 'Conversion failed', 'error');
  } finally {
    setLoading(false);
  }
}
...
function displayAnalysisResults(analysis) {
  ...
  const checkedTotal = summary.checkedChannels;
  const livePercent = checkedTotal > 0 ? (summary.liveChannels / checkedTotal) * 100 : 0;
  const deadPercent = checkedTotal > 0 ? (summary.deadChannels / checkedTotal) * 100 : 0;
  ...
  if (analysis.groups.length > 12) {
    categoriesGrid.innerHTML += `<span class="category-tag">+${analysis.groups.length - 12} more</span>`;
  }
...
function renderChannels(channels, filter) {
  ...
  if (filter === 'live') {
    filtered = channels.filter(c => c.status === 'live');
  } else if (filter === 'dead') {
    filtered = channels.filter(c => c.status === 'dead');
  }
  ...
  if (filtered.length > displayedChannels) {
...
function filterChannels(filter) {
  ...
  if (currentAnalysis) {
    displayedChannels = 20;
    renderChannels(currentAnalysis.channels, filter);
  }
}
...
function showMoreChannels() {
  ...
  if (currentAnalysis) {
    renderChannels(currentAnalysis.channels, activeFilter);
  }
}
...
// ============================================
// Download Functions (Client-Side PDF)
// ============================================
...
async function downloadReport() {
  ...
  try {
    ...
    hideStatus();
    showStatus('Success', 'Report downloaded successfully', 'success');
    setTimeout(hideStatus, 3000);
    
  } catch (error) {
    showStatus('Error', 'Failed to generate report: ' + error.message, 'error');
  }
}
...
async function downloadPdf() {
  ...
  try {
    ...
    hideStatus();
    showStatus('Success', 'PDF downloaded successfully', 'success');
    setTimeout(hideStatus, 3000);
    
  } catch (error) {
    showStatus('Error', 'Failed to generate PDF: ' + error.message, 'error');
  }
}
...
function showStatus(title, message, type = 'info') {
  ...
  let iconSvg = '';
  if (type === 'error') {
    iconSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" width="20" height="20">
      <circle cx="12" cy="12" r="10"/>
      <line x1="15" y1="9" x2="9" y2="15"/>
      <line x1="9" y1="9" x2="15" y2="15"/>
    </svg>`;
  } else if (type === 'success') {
    iconSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2" width="20" height="20">
      <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/>
      <polyline points="22 4 12 14.01 9 11.01"/>
    </svg>`;
  } else {
    iconSvg = `<svg viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" stroke-width="2" width="20" height="20">
      <circle cx="12" cy="12" r="10"/>
